...
---
## #065 — July 12, 2026 — Augmentation goes fetch-first on Haiku (prompt v6; revises #047/#048's search-first pipeline)

**Decision:** Rebuild `/api/augment` around fetching instead of searching, after two augmentations cost $1.12 (~56¢ each). The new pipeline: (1) resolve the coffee's product-page URL — the admin now pastes it in the **Verify queue at approval time** (written to `coffees.source_url`; the approve button becomes "Approve & augment" and fires augmentation immediately), falling back to ONE Haiku web search (`max_uses: 1`, domain-narrowed via a sibling coffee of the same roaster) only for the legacy backlog, else reporting `needs_url` instead of guessing; (2) fetch that page server-side for free (robots.txt-respecting, SSRF-guarded, 2 MB cap; JSON-LD product blocks extracted before a zero-dep HTML→text strip capped at 40k chars); (3) one **Claude Haiku 4.5** call with no tools extracts facts + commerce from the bounded page text (prompt v6 keeps v5's rules — null-when-unverified, region vocabulary, exact-price/never-construct-URLs — and adds an untrusted-`<page>`-data wrapper and a same-coffee sanity check). Every run now records its `usage` and an `est_cost_usd` in `raw_response` and returns them; the admin UI shows per-run cost, batch spend, and a "needs URL" list. Proposal shape, review flow, and response contract are unchanged.

**Why (the cost math):** The search *fee* was never the problem — $10/1,000 searches ≈ 2¢/coffee. Per Anthropic's docs, search **results bill as input tokens "in search iterations executed during a single turn and in subsequent conversation turns"** — every internal step re-reads the accumulated result blocks, and each `pause_turn` replay re-sent the whole conversation uncached. ~56¢/coffee ≈ 150K+ input tokens on Sonnet at $3/MTok. Fetching the page ourselves is free; a bounded ~10–15K-token page on Haiku ($1/$5 per MTok) prices extraction at ~1–3¢. The earlier "costs are near-zero" claim was made without ever logging `usage` — which is why cost observability ships inside this change, not after it.

**Alternatives considered:** (1) *Stay on Sonnet + search, tune the prompt* — no prompt fixes the architecture of paying to re-read search results. (2) *Manual URL only, no search ever* — cheapest and fully deterministic, but strands the existing un-URL'd catalog; kept as the dominant path (admin paste at approval) with the one-shot search as backlog fallback. (3) *Third-party retail/affiliate API* — still the eventual Approach C (#048) if Palato scales; premature now. (4) *Batch API (50% off)* — deferred; only pays once batch runs get large.

**Tradeoffs accepted:** (a) Haiku extraction quality vs Sonnet is unproven — `model_version`/`prompt_version` on every row make the v5-Sonnet vs v6-Haiku comparison the first job of the planned augmentation-accuracy eval. (b) robots.txt-disallowed and bot-blocked pages now surface as fetch failures routed to the manual backlog (#048 posture) rather than being silently worked around. (c) One more admin step per new coffee (finding the product URL) — accepted by Jesse explicitly: he'd rather paste the URL than pay for search; the search fallback still covers batch runs. (d) JSON-LD + regex text-strip is cruder than a real HTML parser — zero new dependencies was the deliberate choice; revisit only if extraction accuracy says so.

**Verification:** build + lint; local script exercising the fetch/strip/robots/SSRF path against real roaster pages (no API key needed); end-to-end on the Vercel deployment — approve-with-URL from the Verify queue, a legacy no-URL coffee, and a `costUsd` cross-check against the Anthropic console.

**Metric:** `raw_response.est_cost_usd` per augmentation row — target ≤ $0.03 on the URL'd path, ≤ $0.06 with the search fallback (vs $0.56 baseline). Watched alongside per-field accuracy once the eval lands.
